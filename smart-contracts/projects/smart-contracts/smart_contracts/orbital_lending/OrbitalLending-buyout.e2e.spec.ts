...
import { calculateDisbursement, computeBuyoutTerms, getCollateralBoxValue, getLoanRecordBoxValue } from './testing-utils'
...
describe('orbital-lending Testing - deposit / borrow', async () => {
  ...
  test('Buyout loan - algo Lending Contract', async () => {
    ...
    // Assertions
    expect(r.eligible).toBe(true) // or false depending on CR
    // Debtor must repay this much base token in group:
    expect(r.debtRepayAmountBase).toBeGreaterThan(0n)
    // Buyer must pay exactly this many xUSD (buyout token) as premium:
